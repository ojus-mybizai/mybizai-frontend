  ...
  return (
    <div className={`mb-3 ${compact ? '' : 'rounded-lg border border-border-color overflow-hidden'}`}>
      ...
      >
        ...
        {/* Add button */}
        <button
          className="flex items-center gap-1 rounded-md bg-primary/10 px-2.5 py-1 text-xs font-medium text-primary hover:bg-primary/20 transition-colors opacity-80 group-hover:opacity-100"
          onClick={(e) => {
            e.stopPropagation();
            setShowCreate(true);
          }}
            ...
            <div className="max-h-[55vh] space-y-4 overflow-y-auto p-5">
              {createFields.map((f) => (
                <div key={f.id}>
                  ...
                  {renderCreateInput(f, createData, setCreateData)}
                </div>
              ))}
            </div>
...
function renderCreateInput(
  ...
  const baseInput =
    'w-full rounded-lg border border-border-color bg-input-bg px-3 py-2 text-sm text-text-primary focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary/20 transition-colors';
