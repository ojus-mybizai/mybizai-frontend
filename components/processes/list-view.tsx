...
export default function ListView({
  ...
  const sorted = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case 'assignee':av = a.assigned_to_name ?? ''; bv = b.assigned_to_name ?? ''; break;
      }
  ...
  const groups = useMemo(() => {
    ...
    for (const e of sorted) {
      ...
      if (groupBy === 'assignee') { key = String(e.assigned_to_id ?? 'none');   label = e.assigned_to_name || 'Unassigned'; }
      ...
      <td className="px-3 py-2 text-text-secondary text-xs">
        {e.assigned_to_name ? (
          <span className="inline-flex items-center gap-1.5">
            <Avatar name={e.assigned_to_name} size="xs" /> <span className="truncate max-w-[100px]">{e.assigned_to_name}</span>
          </span>
        ) : '—'}
      </td>
