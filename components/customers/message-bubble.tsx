...
export function MessageBubble({ message }: { message: MessageWithMeta }) {
  const isUser = message.role === 'user';
  const isAssistant = message.role === 'assistant';
  const isTool = message.role === 'tool' || message.role === 'system';

  return (
    <div className={cx('flex', isUser ? 'justify-start' : 'justify-end')}>
      <div
        className={cx(
      ...
      >
        ...
        {(message.tool_called || message.intent) && (
          <div className="mt-2 flex flex-wrap gap-1.5">
            {message.tool_called && (
              <span className={cx('rounded border px-2 py-0.5 text-xs font-medium', isAssistant ? 'border-white/40 text-white/90' : 'border-border-color bg-bg-secondary/60 text-text-secondary')}>
