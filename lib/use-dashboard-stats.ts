...
async function fetchDashboardData(lmsEnabled: boolean, signal?: AbortSignal): Promise<DashboardData> {
  ...
  const [leadResult, convos, agents] = await Promise.all([
    leadStatsPromise,
    apiFetch<Array<{ id: number; summary?: string | null; last_message_at?: string | null; lead_name?: string | null }>>('/convo/', { method: 'GET', signal }).catch(() => []),
    listAgents().catch(() => []),
  ]);

  const { data: leadStats, error: leadError } = leadResult;
  const leadStatsError = leadError
    ? leadError === 'LMS_DISABLED' ? 'Enable LMS to see lead metrics' : leadError
    : null;
...
export function useDashboardStats(options?: { lmsEnabled?: boolean; enabled?: boolean }) {
  const lmsEnabled = options?.lmsEnabled !== false;
  const enabled = options?.enabled !== false;

  const { data, isLoading } = useQuery({
    queryKey: queryKeys.dashboardStats(lmsEnabled),
    queryFn: ({ signal }) => fetchDashboardData(lmsEnabled, signal),
    enabled,
  });
  ...
  return {
    ...
    leadStatsError: data?.leadStatsError ?? null,
    loading: isLoading,
  };
}
