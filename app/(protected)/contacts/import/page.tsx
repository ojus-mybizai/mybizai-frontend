...
type DuplicateCheckField = 'phone' | 'email' | 'name';
type OnDuplicate = 'skip' | 'update' | 'create_anyway';
type ContactField = 'name' | 'phone' | 'email' | 'priority' | 'source' | 'notes';
            ...
            <div className="flex flex-col items-center gap-1">
              <div
                className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-semibold transition-colors ${
                  done ? 'bg-green-500 text-white' : active ? 'bg-accent text-white' : 'bg-bg-secondary border border-border-color text-text-secondary'
                }`}
              >
                {done ? (
...
const CONTACT_FIELD_LABELS: Record<ContactField, string> = {
  ...
  priority: 'Priority', source: 'Source', notes: 'Notes',
};
const CONTACT_FIELDS: ContactField[] = ['name', 'phone', 'email', 'priority', 'source', 'notes'];

// ─── Step 1 — Settings ────────────────────────────────────────────────────────

function SettingsStep({
  settings, onChange, onNext, saving,
}: {
  settings: ContactImportSettings | null;
  onChange: (s: Partial<ContactImportSettings>) => void;
  onNext: () => Promise<void>;
  saving: boolean;
}) {
  if (!settings) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-text-secondary">
          Choose which fields to check when deciding if an incoming contact already exists.
        </p>
      </div>

      <Section title="Match contacts by">
        {(['phone', 'email', 'name'] as DuplicateCheckField[]).map((f) => (
          <CheckRow
            ...
            description={
              f === 'phone' ? 'Normalises numbers (ignores spaces, dashes, country codes) before comparing'
              : f === 'email' ? 'Case-insensitive comparison'
      ...
      <Section title="Duplicate scope">
        <RadioRow name="scope" value="business" current={settings.duplicate_scope}
          onChange={(v) => onChange({ duplicate_scope: v as 'business' | 'employee' })}
          label="Whole business" description="Check across every contact in your business regardless of who it's assigned to" />
        <RadioRow name="scope" value="employee" current={settings.duplicate_scope}
          onChange={(v) => onChange({ duplicate_scope: v as 'business' | 'employee' })}
          label="Per employee" description="Only check contacts assigned to the person doing the import" />
      </Section>
      ...
      <Section title="Cross-employee rules" subtitle="When the same phone / email exists under a DIFFERENT employee, should it count as a duplicate?">
        <CheckRow
          ...
          description="If unchecked, two employees can have contacts with the same phone number" />
        <CheckRow
          ...
          description="If unchecked, two employees can have contacts with the same email address" />
      </Section>

      <Section title="When a duplicate is found">
        {([
          ['skip', 'Skip row', 'Do not import the row — leave the existing contact untouched'],
          ['update', 'Update existing', 'Overwrite fields in the existing contact with data from the file'],
          ['create_anyway', 'Import anyway', 'Create a new contact even if a match is found'],
        ] as [OnDuplicate, string, string][]).map(([val, label, desc]) => (
          <RadioRow key={val} name="on_duplicate" value={val} current={settings.on_duplicate}
            onChange={(v) => onChange({ on_duplicate: v as OnDuplicate })}
            label={label} description={desc} />
        ))}
        <p className="m-4 mt-2 text-[11px] text-text-secondary italic">
...
function UploadStep({ onPreview, loading }: { onPreview: (file: File) => Promise<void>; loading: boolean }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-sm font-semibold text-text-primary mb-1">Upload your contacts file</h3>
        <p className="text-xs text-text-secondary">
      ...
      <div
        ...
        className={`flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed cursor-pointer py-14 transition-colors ${
          dragging ? 'border-accent bg-accent/5' : 'border-border-color bg-bg-secondary hover:border-accent/50 hover:bg-accent/[0.03]'
        }`}
      >
        {loading ? (
          <>
        ...
        <input ref={inputRef} type="file" accept=".csv,.xlsx,.xls,.xlsm" className="hidden"
      ...
      <div className="rounded-lg border border-border-color bg-bg-secondary p-4">
        ...
        <p className="mt-2 text-[11px] text-text-secondary">
          Column names are matched case-insensitively. You can remap columns manually after uploading.
        </p>
      </div>
    </div>
  );
...
function ReviewStep({
  preview, rowActions, onActionChange, onImport, importing, defaultSource, onDefaultSourceChange,
}: {
  ...
  const filteredRows = filterStatus === 'all' ? preview.rows : preview.rows.filter((r) => r.import_status === filterStatus);

  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <SummaryCard label="Total rows"  value={preview.total} color="text-text-primary" />
        <SummaryCard label="New"         value={newCount}      color="text-green-600" />
      ...
      <div className="rounded-lg border border-border-color bg-bg-secondary p-3 flex flex-wrap gap-4 text-xs">
        ...
        <span><span className="font-semibold text-text-secondary">{rowActions.filter((a) => a === 'skip').length}</span> will be skipped</span>
      </div>

      {/* Column mapping */}
      <div>
        <button type="button" onClick={() => setShowMapping((v) => !v)}
        ...
        {showMapping && (
          <div className="mt-3 rounded-lg border border-border-color bg-card-bg p-3 space-y-2">
            <p className="text-xs text-text-secondary">Map each file column to the correct contact field (or ignore it).</p>
            <div className="grid gap-2">
              {localMapping.map((entry, i) => (
                <div key={entry.raw_column} className="flex items-center gap-3 text-xs">
                  <span className="w-40 truncate font-medium text-text-primary" title={entry.raw_column}>{entry.raw_column}</span>
                  ...
                  <select
                    ...
                    value={entry.contact_field ?? ''}
                    onChange={(e) => {
                      const updated = localMapping.map((m, mi) =>
                        mi === i ? { ...m, contact_field: (e.target.value || null) as ContactField | null } : m,
                      );
                  ...
                  >
                    ...
                    {CONTACT_FIELDS.map((f) => (
                      <option key={f} value={f}>{CONTACT_FIELD_LABELS[f]}</option>
                    ))}
                  </select>
                </div>
              ))}
      ...
      <div className="rounded-lg border border-border-color bg-bg-secondary p-3 flex flex-wrap gap-4 items-end">
        <div className="space-y-1">
          ...
          <input type="text" placeholder="e.g. import, referral, website" value={defaultSource}
        ...
        {(['all', 'new', 'duplicate', 'error'] as const).map((f) => (
          <button key={f} onClick={() => setFilterStatus(f)}
            className={`px-3 py-1.5 text-xs font-medium capitalize transition-colors border-b-2 -mb-px ${
              filterStatus === f ? 'border-accent text-accent' : 'border-transparent text-text-secondary hover:text-text-primary'
            }`}>
      ...
      <div className="rounded-xl border border-border-color overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody>
              {filteredRows.map((row: ImportRowPreview) => {
                const action = rowActions[row.row_index];
                const statusMeta = STATUS_LABELS[row.import_status];
                return (
                  <tr key={row.row_index}
                    className={`border-b border-border-color last:border-b-0 hover:bg-bg-secondary/50 ${action === 'skip' ? 'opacity-50' : ''}`}>
                    ...
                    <td className="px-3 py-2 text-text-secondary max-w-[140px]">
                      {row.duplicate_info ? (
                        <div>
                          <span className="font-medium text-text-primary">{row.duplicate_info.name || 'Contact'}</span>
      ...
      {dupCount > 0 && (
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          {(['skip', 'update', 'import'] as RowAction[]).map((a) => (
            <button key={a}
              onClick={() => preview.rows.filter((r) => r.import_status === 'duplicate').forEach((r) => onActionChange(r.row_index, a))}
      ...
      <div className="flex justify-end pt-2">
        ...
        >
          ...
          Import {willImport + willUpdate} contact{(willImport + willUpdate) !== 1 ? 's' : ''}
        </button>
      </div>
    </div>
  );
...
function DoneStep({ result, onImportAnother }: { result: ContactImportConfirmResult; onImportAnother: () => void }) {
        ...
        <div className="w-full rounded-lg border border-red-200 bg-red-50 dark:bg-red-950/20 dark:border-red-800 p-3">
          ...
          <div className="space-y-1 max-h-40 overflow-y-auto">
            {result.error_details.map((e, i) => (
              <p key={i} className="text-xs text-red-600 dark:text-red-400">Row {e.row_index + 1}: {e.error}</p>
            ))}
          </div>
        </div>
      )}
...
function CheckRow({ checked, onChange, label, description }: { checked: boolean; onChange: () => void; label: string; description: string }) {
  return (
    <label className="flex cursor-pointer items-start gap-3 px-4 py-3 hover:bg-bg-secondary/50 transition-colors">
      <div className="mt-0.5 flex-none">
        <div className={`h-4 w-4 rounded border-2 flex items-center justify-center transition-colors ${checked ? 'border-accent bg-accent' : 'border-border-color bg-bg-secondary'}`}
          onClick={onChange}>
          {checked && (
...
function RadioRow({ name, value, current, onChange, label, description }: {
  name: string; value: string; current: string; onChange: (v: string) => void; label: string; description: string;
}) {
  const active = value === current;
  return (
    <label className="flex cursor-pointer items-start gap-3 px-4 py-3 hover:bg-bg-secondary/50 transition-colors">
      <div className="mt-0.5 flex-none">
        <div className={`h-4 w-4 rounded-full border-2 flex items-center justify-center transition-colors ${active ? 'border-accent' : 'border-border-color bg-bg-secondary'}`}
...
export default function ImportContactsPage() {
  ...
  const [settings, setSettings] = useState<ContactImportSettings | null>(null);
  ...
  useEffect(() => {
    getImportSettings().then(setSettings).catch(() => {
      setSettings({
        id: 0, business_id: 0,
  ...
  const handlePreview = async (file: File) => {
    ...
    try {
      ...
      setPreview(res);
      setRowActions(res.rows.map((r) => r.row_action));
      setStep(2);
    } catch (err: unknown) {
  ...
  const handleActionChange = useCallback((idx: number, action: RowAction) => {
    setRowActions((prev) => { const next = [...prev]; next[idx] = action; return next; });
  }, []);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await confirmImport({ rows, default_source: defaultSource || null });
      setResult(res);
      setStep(3);
    } catch {
  ...
  const handleImportAnother = () => {
    setStep(0); setPreview(null); setResult(null); setRowActions([]); setError(null);
  };
  ...
  return (
    <div className="flex h-full flex-col gap-4 overflow-y-auto pb-8">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold text-text-primary sm:text-2xl">Import Contacts</h2>
      ...
      <div className="rounded-xl border border-border-color bg-card-bg p-5 md:p-6">
        {step === 0 && (
          <SettingsStep settings={settings} onChange={(p) => setSettings((prev) => prev ? { ...prev, ...p } : prev)}
            onNext={handleSaveSettings} saving={savingSettings} />
        )}
        {step === 1 && <UploadStep onPreview={handlePreview} loading={loadingPreview} />}
        {step === 2 && preview && (
          <ReviewStep preview={preview} rowActions={rowActions} onActionChange={handleActionChange}
            onImport={handleImport} importing={importing}
            defaultSource={defaultSource} onDefaultSourceChange={setDefaultSource} />
        )}
        {step === 3 && result && <DoneStep result={result} onImportAnother={handleImportAnother} />}
      </div>
    </div>
  );
}
