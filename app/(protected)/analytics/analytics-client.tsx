...
export default function AnalyticsClient() {
  const { toISORange } = useDateRangeStore();
  ...
  const fetchSummary = useCallback(async () => {
    setLoading(true);
    setError(null);
    const { start_date, end_date } = toISORange();
    try {
      const [summaryRes, conversationRes] = await Promise.all([
        getAgentsSummary({ start_date, end_date }),
        listConversationAnalytics({ start_date, end_date, limit: 50 }),
      ]);
  ...
  }, [toISORange, startDate, endDate]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <div className="mx-auto max-w-7xl space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-text-secondary">Agent performance and conversation metrics.</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              onClick={async () => {
                ...
                const { start_date, end_date } = toISORange();
                try {
                  const res = await backfillAnalytics({ start_date, end_date, limit: 2000 });
                  setBackfillMessage(`Backfill complete. Processed ${res.processed_sessions ?? 0} sessions.`);
                  await fetchSummary();
                } catch (e) {
        ...
        {!loading && summary && (
          <>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="rounded-xl border border-border-color bg-card-bg px-4 py-3">
                <div className="text-xs font-medium text-text-secondary mb-1">Slow-response conversations (&gt; 10s)</div>
                <div className="text-xl font-semibold text-text-primary">
                  {conversationRows.filter((c) => (c.avg_response_time ?? 0) > 10).length}
                </div>
              </div>
              <div className="rounded-xl border border-border-color bg-card-bg px-4 py-3">
                <div className="text-xs font-medium text-text-secondary mb-1">Escalated</div>
                <div className="text-xl font-semibold text-text-primary">
                  {conversationRows.filter((c) => (c.resolution_status ?? '').toLowerCase() === 'escalated').length}
                </div>
              </div>
              <div className="rounded-xl border border-border-color bg-card-bg px-4 py-3">
                <div className="text-xs font-medium text-text-secondary mb-1">Total tool calls</div>
                <div className="text-xl font-semibold text-text-primary">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="rounded-xl border border-border-color bg-card-bg px-4 py-3">
                ...
                <div className="text-xl font-semibold text-text-primary">{summary.avg_response_time.toFixed(1)}s</div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border-color bg-card-bg p-4">
              ...
              <div className="text-2xl font-semibold text-text-primary">{summary.resolution_rate.toFixed(1)}%</div>
            </div>
            ...
            {summary.top_agents && summary.top_agents.length > 0 && (
              <div className="rounded-xl border border-border-color bg-card-bg overflow-hidden">
                <h3 className="text-sm font-semibold text-text-primary p-4 border-b border-border-color">Top agents</h3>
                    ...
                    <tbody className="divide-y divide-border-color text-sm">
                      {summary.top_agents.map((a) => (
                        <tr key={a.agent_id}>
                          ...
                          <td className="px-4 py-3 text-text-secondary">{a.avg_response_time.toFixed(1)}</td>
                          <td className="px-4 py-3 text-text-secondary">{a.resolution_rate.toFixed(1)}%</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="rounded-xl border border-border-color bg-card-bg overflow-hidden">
              ...
              {conversationRows.length === 0 ? (
                <div className="p-4 text-sm text-text-secondary">No conversation analytics rows for this range yet.</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-border-color">
                    <thead className="bg-bg-secondary text-xs uppercase text-text-secondary">
