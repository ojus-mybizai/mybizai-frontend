...
export default function EditLeadTemplatePage() {
  ...
  useEffect(() => {
    ...
    getLeadTemplate(id)
      .then((t) => {
        ...
        setIntentCategory(t.intent_category ?? '');
        setIntentKeywords(Array.isArray(t.intent_keywords) ? t.intent_keywords.join(', ') : '');
      ...
      .catch((err) => {
        if (!cancelled) setError((err as Error).message ?? 'Failed to load template');
      })
      .finally(() => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!Number.isFinite(id) || !template) return;
    setError(null);
    if (!name.trim()) {
    ...
    return (
      <ProtectedShell>
        <ModuleGuard module="agents">
          <div className="mx-auto max-w-3xl rounded-2xl border border-border-color bg-card-bg p-8 text-center text-sm text-text-secondary">
            {loading ? 'Loading…' : 'Template not found.'}
          </div>
        </ModuleGuard>
      </ProtectedShell>
    );
  ...
  return (
    <ProtectedShell>
        ...
        <div className="mx-auto max-w-3xl space-y-6">
          <div>
            <h2 className="text-xl font-semibold text-text-primary sm:text-2xl">Edit Lead Template</h2>
            <p className="text-sm text-text-secondary">
              Update intent and custom fields for lead extraction.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 rounded-2xl border border-border-color bg-card-bg p-5">
            ...
            <div className="flex gap-3">
              ...
              <button
                type="button"
                onClick={() => router.push('/lead-templates')}
